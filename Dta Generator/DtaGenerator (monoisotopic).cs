        ...
        public void GenerateDtas()
        {
            ...
            try
            {
                ...
                for(int file_index = 0; file_index < rawFilepaths.Count; file_index++)
                {
                    ...
                    raw = (XRAWFILE2Lib.IXRawfile3)new XRAWFILE2Lib.XRawfileClass();
                    string filepath = rawFilepaths[file_index];
                    onStartingFile(new FilepathEventArgs(filepath));
                    raw.Open(filepath);
                    raw.SetCurrentController(0, 1);

                    log = new StreamWriter(Path.Combine(outputFolder, Path.GetFileNameWithoutExtension(filepath) + "_log.txt"));
                    log.AutoFlush = true;
                    ...
                    for(int scan_number = first_scan_number; scan_number <= last_scan_number; scan_number++)
                    {
                        ...
                        if(scan_filter.Contains("@"))
                        {
                            ...
                            int monoisotopic_index = precursor_index.Value;
                            while(containsPeak(precursor_data,
                                precursor_data[(int)RawLabelDataColumn.MZ, monoisotopic_index] - MAXIMUM_ISOTOPIC_SPACING / charge - MASS_TOLERANCE / 1e6 * precursor_data[(int)RawLabelDataColumn.MZ, monoisotopic_index],
                                precursor_data[(int)RawLabelDataColumn.MZ, monoisotopic_index] - MINIMUM_ISOTOPIC_SPACING / charge + MASS_TOLERANCE / 1e6 * precursor_data[(int)RawLabelDataColumn.MZ, monoisotopic_index],
                                charge, 0.1 * precursor_data[(int)RawLabelDataColumn.Intensity, monoisotopic_index], ref monoisotopic_index)) { }
                            ...
                            precursor_mz = precursor_mass_recalibration * precursor_data[(int)RawLabelDataColumn.MZ, monoisotopic_index];

                            if(precursor_data.GetLength(0) > 2)
                            {
                                precursor_sns.Add(scan_number, (precursor_data[(int)RawLabelDataColumn.Intensity, monoisotopic_index] - precursor_data[(int)RawLabelDataColumn.NoiseBaseline, monoisotopic_index]) / precursor_data[(int)RawLabelDataColumn.NoiseLevel, monoisotopic_index]);
                            }
                            else
                            {
                            ...
                            for(int i = precursor_data.GetLowerBound(1); i <= precursor_data.GetUpperBound(1); i++)
                            {
                                if(i != monoisotopic_index)
                                {
                                    if(precursor_data[(int)RawLabelDataColumn.Intensity, i] > precursor_data[(int)RawLabelDataColumn.Intensity, monoisotopic_index])
                                    {
                                        peak_depth++;
                                    }
                                }
                                            ...
                                            if(!clean)
                                            {
                                                for(int reduced_precursor_charge = 1; reduced_precursor_charge <= charge_i - 1; reduced_precursor_charge++)
                                                {
                                                    if(mz >= mzFromMass(precursor_mass - LOW_NEUTRAL_LOSS_CLEANING_WINDOW_DA, reduced_precursor_charge) &&
                                                        mz <= precursor_mz + HIGH_PRECURSOR_CLEANING_WINDOW_MZ)
                                                    {
                                                        clean = true;
                                                        break;
                                                    }
