...
namespace Phosphinator
{
    public class ModificationDictionary : SortedDictionary<string, Modification>
    {
        ...
        public ModificationDictionary(string modsXmlFolder)
            : base()
        {
            string mods_xml_filepath = Path.Combine(modsXmlFolder, "mods.xml");
            if(File.Exists(mods_xml_filepath))
            {
                ReadModificationsFromXmlFile(mods_xml_filepath);
            }

            string[] user_mods_xml_filepaths = Directory.GetFiles(modsXmlFolder, "usermods*.xml");
            if(user_mods_xml_filepaths.Length > 1)
            {
                throw new FileLoadException(@"Multiple usermods*.xml files exist in " + modsXmlFolder);
            }
            if(user_mods_xml_filepaths.Length > 0)
            {
                ReadModificationsFromXmlFile(user_mods_xml_filepaths[0]);
            }
        }

        public void ReadModificationsFromXmlFile(string modsXmlFilepath)
        {
            ...
            foreach(XmlNode mod_node in mods_xml.SelectNodes("/omssa:MSModSpecSet/omssa:MSModSpec", mods_xml_ns))
            {
                ...
                Modification mod = new Modification(mod_name, mod_monomass, mod_type, mod_amino_acid_residues);
